<template lang="pug">
  div.home
    Header(title="Colors")
    .container
      .columns
        .column.is-half.is-offset-one-quarter
            Color(value="rgb(51, 51, 51)")
            Color(value="rgb(246, 249, 252)")
            Color(value="#00d1b2")
            Color(:value=['#000000', '#AAAAAA'])
            Color(:value=['#452393', '#FC3768', '#1FBCD4'])
            Color(:value=['#DD6463', '#027DB9', '#FFFAC5'])
            Color(:value=['#0D5B58', '#D46A1F', '#F4BB12', '#511422'])
            Color(:value=['#50ae96', '#3c78d8'])
            Color(:value=['#f94a7a', '#2ac06d', '#4A9FF9'])
            Color(:value=['#2B97F9'])
            Color(:value=['rgb(38, 166, 154)', 'rgb(0, 215, 68)', 'rgb(245, 157, 0)', 'rgb(255, 110, 64)', 'rgb(233, 30, 99)', 'rgb(255, 0, 198)', 'rgb(157, 0, 239)', 'rgb(30, 161, 255)', 'rgb(0, 198, 197)', 'rgb(83, 109, 254)'])
            #Type(value="Hello world", font="Roboto")
            #Type(value="Nice to meet you", font="Open Sans")
            #Type(value="hi hi hi hi hi Nice to meet you", font="Open Sans")
</template>

<script>
import Header from "@/components/Header.vue";

import Color from "@/components/Color.vue";
import Type from "@/components/Type.vue";

export default {
  name: "home",
  created() {
    document.title = document.location.hostname;
  },
  components: {
    Header,
    Color,
    Type
  }
};
</script>

<style scoped lang="sass"></style>
